...
## Alineación con schema actual (commit `b6c444f`)

Renombres aplicados en [`src/db/schema.ts`](src/db/schema.ts) que el seed debe respetar:

| Tabla      | Antes          | Ahora (columna SQL / campo Drizzle)            |
| ---------- | -------------- | ---------------------------------------------- |
| `usuarios` | `email`        | `correo` / `correo`                            |
| `viajes`   | `pickup_en`    | `fecha_partida` / `fechaPartida`               |
| `viajes`   | `dropoff_en`   | `fecha_llegada` / `fechaLlegada`               |
| `viajes`   | `pickup_lat`   | `origen_latitud` / `origenLatitud`             |
| `viajes`   | `pickup_lng`   | `origen_longitud` / `origenLongitud`           |
| `viajes`   | `dropoff_lat`  | `destino_latitud` / `destinoLatitud`           |
| `viajes`   | `dropoff_lng`  | `destino_longitud` / `destinoLongitud`         |
| `viajes`   | `distancia_km` | `distancia_kilometros` / `distanciaKilometros` |
| `viajes`   | `duracion_min` | `duracion_minutos` / `duracionMinutos`         |

Sin cambios: `usuarios`, `conductores` y `pagos` mantienen el resto de columnas iguales.
...
| #   | viaje_id                               | pasajero  | conductor | estado          | origen_latitud, origen_longitud    | destino_latitud, destino_longitud  | distancia_kilometros | duracion_minutos | tarifa_base (ARS) | mult | tarifa_total (ARS) |
| --- | -------------------------------------- | --------- | --------- | --------------- | ---------------------------------- | ---------------------------------- | -------------------- | ---------------- | ----------------- | ---- | ------------------ |
...
1. Crear [`src/db/seed.ts`](src/db/seed.ts) con inserts vía Drizzle ORM usando los campos actuales del schema (`correo`, `fechaPartida`, `fechaLlegada`, `origenLatitud`, `origenLongitud`, `destinoLatitud`, `destinoLongitud`, `distanciaKilometros`, `duracionMinutos`)
